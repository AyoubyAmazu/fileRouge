        ...
        <div v-else class="container mx-auto max-w-6xl">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 mb-6">
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                    <div class="flex-1">
                        <div class="flex items-center gap-4 mb-4">
                            <div
                                class="h-16 w-16 bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl flex items-center justify-center">
                ...
                <div v-if="event.description" class="mt-6 pt-6 border-t border-gray-100">
                    <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
                        <div
                            class="h-10 w-10 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0 mt-1">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                ...
                <!-- Inscriptions -->
                <div class="flex items-center justify-between gap-3 p-4 bg-yellow-50 rounded-xl shadow-sm">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="font-semibold text-gray-900">{{ event.nb_inscriptions ?? 0 }}</div>
                            <div class="text-sm text-gray-600">Total des inscriptions</div>
                        </div>
                    </div>

                    <button @click="router.push(`/evenement/inscriptions?id=${event.id}`)"
                        class="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded-lg shadow transition duration-200 font-medium flex items-center gap-2">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                <div v-if="event.evenement_jours && event.evenement_jours.length" class="space-y-6">
                    <div v-for="(jour, index) in event.evenement_jours" :key="index"
                        class="border border-gray-200 rounded-2xl p-6 hover:shadow-md transition-shadow duration-200">
                        <!-- En-tête du jour -->
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                            <div class="flex items-center gap-4">
                                <div
                                    class="h-14 w-14 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-xl flex items-center justify-center">
                            ...
                            <div class="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-lg">
                                ...
                                <span class="font-semibold text-blue-900">{{ jour.heure_debut }} - {{ jour.heure_fin
                                    }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-3 mb-4">
                            ...
                            <div v-if="jour.apporter"
                                class="flex items-center gap-2 px-3 py-2 bg-purple-100 text-purple-800 border border-purple-200 rounded-lg text-sm font-medium">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round">
                        ...
                        <div v-if="jour.description" class="bg-gray-50 rounded-xl p-4">
                            <div class="flex items-start gap-3">
                                <div
                                    class="h-8 w-8 bg-gray-200 rounded-lg flex items-center justify-center flex-shrink-0 mt-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                        fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round" class="text-gray-600">
                                        <circle cx="12" cy="12" r="10" />
...
import { countInscriptionByEvent } from '@/modules/Evenement.module/service/inscription.service.js'
...
const fetchEvent = async (id) => {
    try {
        ...
        event.value.nb_inscriptions = await totalInscription(event.value.id)
    } catch (error) {
...
const totalInscription = async (eventId) => {
    try {
        const response = await countInscriptionByEvent(eventId)
        return response.data.count
    } catch (error) {
        console.error('Error counting inscriptions:', error)
        return 0
    }
}
